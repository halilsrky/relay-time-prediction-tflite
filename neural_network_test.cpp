...
// StandardScaler parametreleri (Python'da eğitilen scaler'dan alındı)
...
public:
    // Constructor - gerçek değerler scaler.pkl dosyasından alındı
    StandardScaler() {
        // Gerçek scaler parametreleri
        mean_ = {450.604869f, 0.254636f, 375.504058f, 5676.409131f, 29.291175f, 29.291175f, 48.666744f, 0.024294f, 0.024294f, 0.024294f};
        scale_ = {219.758152f, 0.114944f, 183.131792f, 1466.465562f, 17.208528f, 17.208528f, 27.137492f, 0.153959f, 0.153959f, 0.153959f};
    }
    
    void transform(std::vector<float>& features) {
        for (size_t i = 0; i < features.size(); ++i) {
            features[i] = (features[i] - mean_[i]) / scale_[i];
        }
    }
};
...
public:
    TFLiteNeuralNetwork(const std::string& model_path) {
        ...
        std::cout << "✅ Neural Network modeli yüklendi: " << model_path << std::endl;
    }
    ...
    float predict(std::vector<float> features) {
        ...
        // Input verisini tensöre kopyala
...
int main() {
    std::cout << "🚀 Neural Network TensorFlow Lite C++ Test Başlatılıyor..." << std::endl;
    std::cout << "============================================================" << std::endl;
    
    // Model ve CSV dosya yolları
    const std::string model_path = "tflite_models/tensorflow_nn_model.tflite";
    const std::string csv_path = "input.csv";
    const std::string targets_path = "targets.csv";
    
    // Neural Network modelini yükle
    TFLiteNeuralNetwork nn(model_path);
    ...
    std::cout << "\n" << std::string(60, '=') << std::endl;
    std::cout << "🧠 NEURAL NETWORK TAHMİNLERİ (Scaling ile)" << std::endl;
    std::cout << std::string(60, '=') << std::endl;
    ...
    for (size_t i = 0; i < input_data.size(); ++i) {
        ...
        // Tahmin yap
        float prediction = nn.predict(input_data[i]);
    ...
    std::cout << "✅ Neural Network modeli başarıyla test edildi." << std::endl;
